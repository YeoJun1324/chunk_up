...
class ApiService {
  ...
  static const String _baseUrl = 'https://api.anthropic.com'; // 기본 URL
  ...
  Future<String?> get apiKey async {
    ...
    _cachedApiKey = await _storageService.getString(_apiKeyStorageKey);
    return _cachedApiKey;
  }
  ...
  static Future<String?> getApiKey() async {
    ...
    try {
      ...
      _cachedApiKey = await service.getString(_apiKeyStorageKey);
      return _cachedApiKey;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _executeRequest(
    ...
    try {
      ...
      final Map<String, String> headers = {
        ...
        'x-api-key': apiKey,  // Anthropic API에서 요구하는 헤더
      };
      ...
      final response = await _httpClient.post(
        ...
        const Duration(seconds: 30), // API 요청 타임아웃 단축
        onTimeout: () {
          throw ApiException(
  ...
  Future<Map<String, dynamic>> sendRequest({
    ...
    try {
      ...
      if (e is ApiException) {
        debugPrint('🚨 API 예외: ${e.message}, 상태 코드: ${e.statusCode}');
      } else {
      ...
      if (modelOverride != null) {
        ...
        try {
          ...
          if (_subscriptionService != null) {
            ...
            // _subscriptionService!.status가 TestSubscriptionStatus 타입으로 로깅
            final statusStr = _subscriptionService!.status.toString().split('.').last;
            debugPrint('🤖 내부 구독 서비스로부터 모델 가져옴: $model (구독 상태: $statusStr)');
          }
          // 2. 내부 인스턴스가 없으면 GetIt 사용 시도
            ...
            // subscriptionService.status가 TestSubscriptionStatus 타입으로 로깅
            final statusStr = subscriptionService.status.toString().split('.').last;
            debugPrint('🤖 GetIt에서 구독 서비스로부터 모델 가져옴: $model (구독 상태: $statusStr)');
          }
        } catch (e) {
          debugPrint('⚠️ 구독 서비스 접근 실패, 기본 모델 사용: $e');
          model = SubscriptionConstants.freeAiModel; // 오류 시 haiku 모델로 폴백
        }
      }
  ...
  Future<bool> testApiConnection() async {
    try {
      ...
      // 이제 항상 고정된 API 키 사용
      final key = "";
      debugPrint('🔑 고정된 API 키 사용: ${key.substring(0, 15)}...');

      // API 키 확인 로그
      if (key.length > 20) {
        debugPrint('✅ API 키 형식 확인: 길이=${key.length}, 시작=${key.substring(0, 12)}');
      } else {
        debugPrint('⚠️ API 키 길이가 너무 짧음: ${key.length}');
      }

      final headers = {
          'Content-Type': 'application/json',
          'x-api-key': key,             // 반드시 필요한 헤더
          'anthropic-version': '2023-06-01',
        };

      // 헤더 로깅
      debugPrint('🧪 테스트 API 요청 헤더:');
      headers.forEach((name, value) {
        if (name.contains('api-key') || name.contains('Authorization')) {
          debugPrint('   $name: ${value.substring(0, min(10, value.length))}...');
        } else {
          debugPrint('   $name: $value');
        }
      });

      // 테스트는 Haiku 모델로 실행 (더 빠르고 비용 효율적)
      final testModel = SubscriptionConstants.freeAiModel;
      debugPrint('🤖 테스트에 사용되는 모델: $testModel');
