...
// Model test screen removed - only using Gemini now
import '../../presentation/screens/subscription_screen.dart'; // Add subscription screen
import '../../presentation/screens/exam_export_screen.dart'; // PDF export screen
...
class RouteService {
  ...
  static MainScreen get mainScreenInstance {
    // 기존 인스턴스가 있으면 재사용, 없으면 새로 생성 (GlobalKey 없이)
    _mainScreenInstance ??= const MainScreen();
  ...
  static Map<String, Widget Function(BuildContext)> getAppRoutes({required bool hasApiKey}) {
    return {
      ...
      // Model test route removed - only using Gemini now
      RouteNames.subscription: (context) => const SubscriptionScreen(), // Added subscription route
      '/premium-export': (context) => const ExamExportScreen(), // PDF export route
    };
  }
...
class _MainScreenState extends State<MainScreen> {
  ...
  // 탭 설정 메서드 (향후 필요시 Provider로 관리 가능)
