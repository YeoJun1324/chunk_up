...
class ErrorService {
  ...
  void handleError(
    ...
    // 로깅
    _logError(error, stackTrace ?? StackTrace.current, operation, errorInfo);
  }

  /// Context와 함께 에러를 처리하는 메서드
  Future<T> handleErrorWithContext<T>({
  ...
  Future<void> handleVoidError({
    ...
    return handleVoidErrorWithContext(
      action: action,
      context: context,
      operation: operation,
      showDialog: showDialog,
  ...
  Future<void> handleVoidErrorWithContext({
    required Future<void> Function() action,
    required BuildContext context,
    String? operation,
    bool showDialog = true,
